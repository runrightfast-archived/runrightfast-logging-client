...
(function() {
	...
	var logger = log4js.getLogger('runrighfast-logging-client');
	...
	var validateConfig = function(config) {
		...
		if (!lodash.isUndefined(config.batch)) {
			if (lodash.isBoolean(config.batch)) {
				...
				config.batch = Hoek.applyToDefaults(defaultBatchOptions, config.batch);
		...
		assert(lodash.isString(config.logLevel) && lodash.contains(validLogLevels, config.logLevel.toUpperCase()),
				'logLevel must be a String set to one of the following values : [DEBUG | INFO | WARN | ERROR]');
		logger.setLevel(config.logLevel.toUpperCase());
	...
	var getConfig = function(options) {
		...
		var config = Hoek.applyToDefaults(defaultOptions, _options);
		// Hoek.applyToDefaults skips over functions - so we need to copy these
		// over manually
		if (_options.errorCallback) {
			config.errorCallback = _options.errorCallback;
		}
